...
function renderQuizCategorySelection(container) {
    ...
    container.innerHTML = `
        <div class="quiz-intro" style="text-align: center; padding: 20px;">
            <h2 style="font-family: 'Cairo', sans-serif; color: var(--primary-color);">📝 Test your knowledge</h2>
            <p style="color: #666; margin-bottom: 20px;">Choose a category to practice</p>
        </div>
...
function renderQuizModesSelection(container) {
    container.innerHTML = `
        <div class="quiz-modes-view" style="padding: 20px; text-align: center; animation: fadeIn 0.5s;">
            <button class="back-btn" onclick="selectedQuizCategory=null; renderView();" style="float: right; background: #eee; border: none; padding: 8px 15px; border-radius: 10px; cursor: pointer;"><i class="fas fa-arrow-left"></i> Back</button>
            <h2 style="font-family: 'Cairo', sans-serif; margin-bottom: 30px; clear: both;">Choose a quiz mode: ${selectedQuizCategory}</h2>
            ...
            <div class="modes-container" style="display: flex; flex-direction: column; gap: 20px; max-width: 400px; margin: 0 auto;">
                <div class="mode-card" onclick="startQuizMode('flashcards')" style="background: white; padding: 25px; border-radius: 20px; box-shadow: 0 5px 15px rgba(0,0,0,0.08); cursor: pointer; transition: transform 0.2s;">
                    ...
                    <h3 style="font-family: 'Cairo', sans-serif;">Flashcards</h3>
                    <p style="font-size: 14px; color: #777;">Cards show German and flip to reveal the Arabic translation</p>
                </div>
                ...
                <div class="mode-card" onclick="startQuizMode('mcq')" style="background: white; padding: 25px; border-radius: 20px; box-shadow: 0 5px 15px rgba(0,0,0,0.08); cursor: pointer; transition: transform 0.2s;">
                    <div style="font-size: 40px; margin-bottom: 10px;">🎯</div>
                    <h3 style="font-family: 'Cairo', sans-serif;">Multiple choice (MCQ)</h3>
                    <p style="font-size: 14px; color: #777;">Choose the correct meaning from 4 options</p>
                </div>
                ...
                <div class="mode-card" onclick="startQuizMode('spelling')" style="background: white; padding: 25px; border-radius: 20px; box-shadow: 0 5px 15px rgba(0,0,0,0.08); cursor: pointer; transition: transform 0.2s;">
                    ...
                    <h3 style="font-family: 'Cairo', sans-serif;">Spelling</h3>
                    <p style="font-size: 14px; color: #777;">Write the German word correctly</p>
                </div>
            </div>
        </div>
    `;
    ...
    container.innerHTML = `
        <div class="quiz-container-active" style="padding: 15px; animation: slideIn 0.4s;">
            <div class="quiz-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                ...
                <button onclick="selectedQuizMode=null; renderView();" style="background: none; border: none; color: #999; cursor: pointer;"><i class="fas fa-times"></i> Exit</button>
            </div>
            ${modeHtml}
        </div>
    `;
            ...
            <div class="card" onclick="this.classList.toggle('flipped')" style="height: 100%; width: 100%;">
                <div class="card-inner">
                    <div class="card-front ${word.art}" style="display: flex; flex-direction: column; justify-content: center; align-items: center; gap: 20px;">
                        ...
                        <p style="font-size: 14px; color: rgba(255,255,255,0.7); position: absolute; bottom: 20px;">Tap to flip 👆</p>
                    </div>
                    <div class="card-back" style="display: flex; flex-direction: column; justify-content: center; align-items: center; gap: 20px;">
                        ...
                        <button class="btn-audio" onclick="event.stopPropagation(); playWordAudio(${word.id})">🔊 Listen</button>
                    </div>
                </div>
            </div>
        </div>
        <div style="text-align: center; margin-top: 30px;">
            <button class="next-btn" onclick="nextQuizQuestion()">Next word ➡️</button>
        </div>
    `;
}
    ...
    return `
        <div class="mcq-quiz-view">
            <div class="question-box" style="background: white; padding: 40px 20px; border-radius: 20px; text-align: center; margin-bottom: 25px; box-shadow: 0 5px 15px rgba(0,0,0,0.05);">
                ...
                <p style="color: #888; margin-top: 10px;">What does this word mean?</p>
            </div>
            <div class="quiz-options">
                ${optionsHtml}
            </div>
            ${quizAnswered ? `<div style="text-align: center; margin-top: 25px;"><button class="next-btn" onclick="nextQuizQuestion()">Next ➡️</button></div>` : ''}
        </div>
    `;
}
    ...
    return `
        <div class="spelling-quiz-view">
            <div class="question-box" style="background: white; padding: 30px 20px; border-radius: 20px; text-align: center; margin-bottom: 25px; box-shadow: 0 5px 15px rgba(0,0,0,0.05);">
                ...
                <p style="color: #888; margin-top: 10px;">Write the German word</p>
            </div>
            
            <div class="input-container" style="max-width: 400px; margin: 0 auto;">
                <input type="text" id="spelling-input" value="${spellingInput}" placeholder="Type here..." 
                ...
                ${quizAnswered ? `
                    <div class="feedback-spelling" style="margin-top: 20px; text-align: center; padding: 15px; border-radius: 15px; background: ${spellingInput.toLowerCase().trim() === word.word.toLowerCase().trim() ? '#eafaf1' : '#fdedec'};">
                        <p style="font-weight: bold; color: ${spellingInput.toLowerCase().trim() === word.word.toLowerCase().trim() ? '#27ae60' : '#c0392b'};">
                            ${spellingInput.toLowerCase().trim() === word.word.toLowerCase().trim() ? '✅ Correct answer!' : '❌ Wrong answer!'}
                        </p>
                        <p style="margin-top: 5px;">Correct answer: <span style="font-weight: bold; font-size: 20px;">${word.art ? word.art + ' ' : ''}${word.word}</span></p>
                        <button class="btn-audio" onclick="playWordAudio(${word.id})" style="margin-top: 10px;">🔊 Listen</button>
                    </div>
                ` : `
                    <button class="start-quiz-btn" onclick="checkSpelling('${word.word.replace(/'/g, "\\'")}')" style="width: 100%; margin-top: 20px;">Check ✅</button>
                `}
            </div>
...
function renderQuizResult(container) {
    ...
    let message = percentage >= 80 ? 'Excellent work!' : percentage >= 60 ? 'Good job, keep practicing!' : 'No problem, try again!';
    ...
    container.innerHTML = `
        <div class="quiz-result" style="text-align: center; padding: 40px 20px; animation: scaleIn 0.5s;">
            ...
            <h2 style="font-family: 'Cairo', sans-serif;">Quiz complete!</h2>
            ...
            <p class="result-percent" style="font-size: 24px; color: #777; margin-bottom: 30px;">Success rate: ${percentage}%</p>
            
            <div style="display: flex; flex-direction: column; gap: 15px; max-width: 300px; margin: 0 auto;">
                <button class="start-quiz-btn" onclick="startQuizMode('${selectedQuizMode}')">Retry quiz 🔄</button>
                <button class="back-btn" onclick="selectedQuizMode=null; renderView();" style="background: #f0f0f0; border: none; padding: 12px; border-radius: 25px; cursor: pointer; font-weight: bold;">Change mode ⚙️</button>
                <button class="back-btn" onclick="selectedQuizCategory=null; selectedQuizMode=null; renderView();" style="background: none; border: none; color: #888; cursor: pointer;">Back to categories</button>
            </div>
        </div>
    `;
}
