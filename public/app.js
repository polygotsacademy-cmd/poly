...
function renderWordsView(container) {
    const categories = ['Alle', ...new Set(words.map(w => w.cat))];
    const catHtml = `<div class="categories">
        ${categories.map(cat => `<button class="cat-btn ${currentCategory === cat ? 'active' : ''}" onclick="setCategory('${cat}')">${cat}</button>`).join('')}
    </div>`;
    
    const filteredWords = words.filter(w => {
        const matchesCat = currentCategory === 'Alle' || w.cat === currentCategory;
        const search = document.getElementById('searchInput').value.toLowerCase();
        const matchesSearch = w.word.toLowerCase().includes(search) || w.ar.includes(search);
        return matchesCat && matchesSearch;
    });

    const cardsHtml = `<div class="cards-container">
        ${filteredWords.map(w => `
    ...
    container.innerHTML = catHtml + cardsHtml;
}
